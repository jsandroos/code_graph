...
class PythonParser:
    ...
    def _collect_definitions(self, tree: ast.AST) -> None:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                node_id = self._make_id("Class", node.name, self.current_file)
                self.defined_names[node.name] = node_id
            elif isinstance(node, ast.FunctionDef) or isinstance(node, ast.AsyncFunctionDef):
                # Only top-level functions (not methods)
                node_id = self._make_id("Function", node.name, self.current_file)
                self.defined_names[node.name] = node_id
    ...
    def _visit_module(self, tree: ast.Module, file_id: str) -> None:
        ...
        for node in tree.body:
            if isinstance(node, ast.ClassDef):
                self._visit_class(node, file_id)
            elif isinstance(node, ast.FunctionDef) or isinstance(node, ast.AsyncFunctionDef):
                self._visit_function(node, file_id, is_method=False)
            # Extract module-level calls (e.g., class decorators, global assignments)
            self._extract_calls_from_node(node, file_id)
    
    def _visit_class(self, node: ast.ClassDef, parent_id: str) -> None:
        """Visit a class definition."""
        class_id = self._make_id("Class", node.name, self.current_file)
        ...
        for item in node.body:
            ...
            if not isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                self._extract_calls_from_node(item, class_id)
        
        self.current_class_stack.pop()
    
    def _visit_function(self, node: ast.FunctionDef, parent_id: str, is_method: bool) -> None:
        ...
        if is_method:
            ...
            display_name = parent_class if (node.name == "__init__" and parent_class) else node.name
            func_id = self._make_id("Method", node.name, self.current_file, parent_class)
        else:
            node_type = "Function"
            parent_class = None
            display_name = node.name
            func_id = self._make_id("Function", node.name, self.current_file)
            # Update defined_names for functions
            self.defined_names[node.name] = func_id
    ...
    def _extract_calls_from_node(self, node: ast.AST, caller_id: str) -> None:
        """Extract function calls from a node (for class-level or module-level code)."""
    ...
    def _handle_call(self, call_node: ast.Call, caller_id: str) -> None:
        ...
        if isinstance(func, ast.Name):
            ...
            if callee_id:
                ...
                callee_node = next((n for n in self.nodes if n.id == callee_id), None)
                if callee_node and callee_node.type == "Class":
                    self.edges.append(Edge(
                        source_id=caller_id,
                        target_id=callee_id,
                        type="instantiates"
            ...
            resolved = False
            # Try to resolve the method locally
            for node in self.nodes:
                if node.type == "Method" and node.name == method_name:
                    self.edges.append(Edge(
                        source_id=caller_id,
                        target_id=node.id,
                        type="calls"
